...
sources; two real go/no-go decisions remain. None of this research is
aarch64-specific except where explicitly marked (Crystal's official
releases cover both aarch64 and x86_64 identically), so it's
reproduced here rather than just pointed at the aarch64 tree.
...
- Current upstream is `1.21.0` (2026-07-16) — this project's official
  ...
  plus `-bundled` variants). Confirmed live (on the aarch64 tree): the
  `bin/crystal` inside the aarch64 tarball is a real ELF aarch64
  binary, **fully statically linked** (needs nothing at runtime beyond
  loading itself), and `bin/shards` (Crystal's package manager,
  matching Nim's `nimble` role) ships in the same tarball — no
  separate install needed. The `bundled` vs. non-bundled tarballs
  turned out to have byte-identical `bin/crystal` (same BuildID) in
  this release, so the distinction doesn't matter for this Dockerfile
  either way — plain `linux-{aarch64,x86_64}.tar.gz` is sufficient.
  **No source build needed at all** — unlike Nim (Step 15) or Step
  11.8's Chez/Gambit/Gerbil, this is a straightforward
  prebuilt-binary extraction on both trees, the simplest packaging
  story of any step in this section so far.
- **Compile-time (not just build-time) runtime deps, confirmed live on
  the aarch64 tree by actually compiling programs against a bare
  `ubuntu:26.04` + `crystal` tarball with nothing else installed (not
  independently re-verified on x86_64/24.04):** `gcc`, `libc6-dev`,
...
that does exist is musl, not glibc; **notably there's no glibc x86_64
Linux binary either**, so this tree would need a from-source build
regardless of architecture). Building from source (`shards build
crystalline --release ...`) needs the Crystal compiler + shards
...
1. **crystalline at all, given it needs a from-source build (several
   minutes to build, not just download-and-extract, on *both* trees
   this time — there's no glibc Linux binary of any architecture) on
   top of its own documented architectural limitations and the
   maintainer-burnout signal above** — vs. skipping `+lsp` entirely
   and relying on `crystal-mode`'s own built-in
   `crystal-def-jump`/`crystal-tool-imp` non-LSP lookup, which needs
   nothing beyond the compiler itself. Crystal is the first language
   in this file where "skip LSP, use the language's own built-in
   tooling" is a real, not just theoretical, option — worth weighing
   rather than defaulting to `+lsp` out of habit.
...
are implemented (mirrored from the aarch64 tree), but a bats hang was
found on that tree — root-caused and a fix already designed, just not
built yet. See this tree's own DECISIONLOG.md "Multi-backend Geiser
(`+chez +gambit +guile`) hangs the whole daemon on ambiguous `.scm`
files" entry (points to the aarch64 tree's DECISIONLOG.md for the full
writeup). This is elisp/Geiser behavior, not architecture-specific, so
it applies here too even though only reproduced live on aarch64. Not
done until that's built, `smoketest.bats` runs clean end-to-end
(aarch64), and GUI verification per this section's own **Verify** note
below.
...
lines. None of this research is aarch64-specific (Doom's module source
is read at a commit pinned identically on both trees, and both trees'
apt versions were checked live directly), so it's reproduced here
rather than just pointed at the aarch64 tree, unlike the Chez/Gambit/
Gerbil image builds themselves.
...
- **Source build is a bigger lift than Chez/Gambit's, confirmed via
  ...
  running fine on the aarch64 tree), but worth a sanity check on that
  tree before committing to a source build there; x86_64 itself has no
  architecture concern either way.
